...
"""
    ...
    try:
        ...
        print(f"- 사용 토큰: {response.usage.total_tokens if hasattr(response, 'usage') and response.usage else '정보 없음'}")
        ...
        if hasattr(e, 'response'):
            print(f"HTTP 상태 코드: {e.response.status_code if hasattr(e.response, 'status_code') else '알 수 없음'}")
            print(f"응답 내용: {e.response.text if hasattr(e.response, 'text') else '없음'}")
